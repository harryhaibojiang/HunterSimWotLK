...
function updateSpellCDs(spell) {
    ...
    if (!!USED_SPELLS.multishot)   {
        ...
        let spell_cd = USED_SPELLS.multishot.base_cd - glyph_cd;
    ...
    if (!!USED_SPELLS.chimerashot)   {
        ...
        let spell_cd = USED_SPELLS.chimerashot.base_cd - glyph_cd;
    ...
    if (!!USED_SPELLS.blackarrow)   {
        let spell_cd = USED_SPELLS.blackarrow.base_cd - talents.resourcefulness * 10;
    ...
    if (!!USED_SPELLS.explosivetrap)   {
        let spell_cd = USED_SPELLS.explosivetrap.base_cd - talents.resourcefulness * 10;
    ...
    if (!!USED_SPELLS.immolatetrap)   {
        let spell_cd = USED_SPELLS.immolatetrap.base_cd - talents.resourcefulness * 10;
    ...
    if (!!USED_SPELLS.frosttrap)   {
        let spell_cd = USED_SPELLS.frosttrap.base_cd - talents.resourcefulness * 10;
        USED_SPELLS.frosttrap.cd = (spell === 'frosttrap') ? spell_cd : Math.max(USED_SPELLS.frosttrap.cd - steptime, 0);
    }
    if (!!USED_SPELLS.snaketrap)   {
        let spell_cd = USED_SPELLS.snaketrap.base_cd - talents.resourcefulness * 10;
...
function steadyShotCalc(range_wep, combatRAP) {
    ...
    let temp_sum = sniper_training + ferocious_insp;
    let specials_mod = (steadymod_1 > 0) ? (1 + steadymod_1 + temp_sum) * (1 + steadymod_2) : (1 + steadymod_2 + temp_sum);
...
function killShotCalc(range_wep, combatRAP) {
    ...
    let wepDmg = (combatRAP * 0.4 + range_wep.ammodps * range_wep.speed + dmg + range_wep.flatdmg + SPELLS.killshot.ranks.rankdmg) * 2;
    let shotDmg = (wepDmg) * range_wep.basedmgmod * combatdmgmod * physdmgmod;
    return shotDmg;
}
...
function petAutoCalc(){
    ...
    let cobra_reflex = (pet_talents.cobra_reflex > 0) ? 1 -  (pet_talents.cobra_reflex - 1) / 2 : 1;
    let autoDmg = (dmg + apbonus) * basedmgmod * physdmgmod * cobra_reflex;
    // debugging
    //let minauto = (BasePet.MinDmg + apbonus) * basedmgmod * physdmgmod * cobra_reflex;
    //let maxauto = (BasePet.MaxDmg + apbonus) * basedmgmod * physdmgmod * cobra_reflex;
